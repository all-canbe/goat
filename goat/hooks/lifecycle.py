...
class CommandHandler(HookHandler):
    ...
    def _build_env(self, context: HookContext, extra: Any = None) -> dict:
        ...
        if isinstance(extra, ToolHookInput):
            ...
            file_path = (
                extra.arguments.get("file_path")
                or extra.arguments.get("filepath")
                or extra.arguments.get("directory")
                or extra.arguments.get("path")
                or extra.target_path
                or ""
            )
            if file_path:
                env["HOOK_FILE_PATH"] = file_path
...
class HookLifecycleSystem:
    ...
    async def on_session_start(self, resume: bool = False) -> list[dict]:
        self.current_context = HookContext(
    ...
    async def on_post_tool_use(self, tool_name: str, arguments: dict, result_data: str = "") -> list[dict]:
    ...
    async def on_stop(self, reason: str = "completed") -> list[dict]:
        self.current_context.event = "stop"
        self.current_context.reason = reason
        return await self.registry.dispatch(HookEvent.Stop, self.current_context)
